	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.chufadi" placeholder="出发地"
								clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.mudedi" placeholder="目的地"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('lvyougonglve','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('lvyougonglve','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('lvyougonglve','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('lvyougonglve','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="景点图片" width="120" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<div v-if="scope.row.jingdiantupian">
							<el-image v-if="scope.row.jingdiantupian.substring(0,4)=='http'" preview-teleported
								:preview-src-list="[scope.row.jingdiantupian.split(',')[0]]"
								:src="scope.row.jingdiantupian.split(',')[0]" style="width:100px;height:100px"></el-image>
							<el-image v-else preview-teleported
								:preview-src-list="[$config.url+scope.row.jingdiantupian.split(',')[0]]"
								:src="$config.url+scope.row.jingdiantupian.split(',')[0]" style="width:100px;height:100px">
							</el-image>
						</div>
						<div v-else>无图片</div>
					</template>
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('lvyougonglve','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('lvyougonglve','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.chufadi&&searchQuery.value.chufadi!=''){
			params['chufadi'] = '%' + searchQuery.value.chufadi + '%'
		}
		if(searchQuery.value.mudedi&&searchQuery.value.mudedi!=''){
			params['mudedi'] = '%' + searchQuery.value.mudedi + '%'
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
	...
	const commentClick=(id)=>{
		context?.$router.push('/discusslvyougonglve?refid=' + id)
	}
	//初始化
	const init = () => {
		getList()
					...
					td {
						...
						.cell {
							...
							.el-button--primary {
								...
								margin: 0 5px 5px 0;
								color: #01b70e;
								background: url() no-repeat left center,#01b70e10;
								width: auto;
								font-size: 14px;
								height: 24px;
							}
							...
							.el-button--info {
								...
								margin: 0 5px 5px 0;
								color: #0172b7;
								background: url() no-repeat left center,#0172b710;
								width: auto;
								font-size: 14px;
								line-height: 24px;
								height: 24px;
							...
							.el-button--danger {
								...
								margin: 0 5px 5px 0;
								color: #f51403;
								background: url() no-repeat left center,#f5140310;
								width: auto;
								font-size: 14px;
								height: 24px;
							}
							...
							.el-button--success {
								...
								margin: 0 5px 5px 0;
								color: #f3842c;
								background: url() no-repeat left center,#f3842c20;
								width: auto;
								font-size: 14px;
								height: 24px;
							}
							...
							.el-button--warning {
								...
								margin: 0 5px 5px 0;
								color: #cd0456;
								background: url() no-repeat left center,#cd045610;
								width: auto;
								font-size: 14px;
								height: 24px;
							}
