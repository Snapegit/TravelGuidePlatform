	...
	<div>
		<div class="home_box">
			...
			<div class="recomList_view">
				<div class="recomList_title">旅游攻略推荐</div>
				<div class="recommend_list_two">
					<mySwiper :data="lvyougonglveRecomList" :type="3"
						:loop="false"
						:navigation="true"
						:pagination="false"
						:paginationType="2"
						...
						<template #default="scope">
							<div class="recommend_item animation_box" @click="detailClick('lvyougonglve',scope.row.id)">
								<div class="recommend_img_box">
									<img class="recommend_img" v-if="isHttp(scope.row.jingdiantupian)" :src="scope.row.jingdiantupian.split(',')[0]" alt="">
									<img class="recommend_img" v-else :src="scope.row.jingdiantupian?$config.url + scope.row.jingdiantupian.split(',')[0]:''" alt="">
								</div>
				...
				<div class="recommend_more_view" @click="moreClick('lvyougonglve')">
			...
			<div class="newsList_view">
				<div class="newsList_title">旅游资讯</div>
...
<script setup>
	...
	//旅游攻略推荐
	const lvyougonglveRecomList = ref([])
	const getlvyougonglveRecomList = () => {
		let autoSortUrl = 'lvyougonglve/autoSort'
		if(context?.$toolUtil.storageGet('frontToken')){
			autoSortUrl = "lvyougonglve/autoSort2"
		}
		context?.$http({
			url: autoSortUrl,
			method: 'get',
			...
			lvyougonglveRecomList.value = res.data.data.list
		})
	}
	//旅游资讯弹窗
			...
			.news_item {
				...
				.news_content {
					...
					.news_title {
						...
						overflow: hidden;
						white-space: nowrap;
						background: url(http://clfile.zggen.cn/20240123/e664a8ade7064178b06105b4d98da3ca.png) no-repeat left center;
						font-weight: 500;
						width: 100%;
						font-size: 14px;
						text-overflow: ellipsis;
